...
void plot(int ts)
{
   ...
   if (!my_pe) {
      ...
      for (n = 0; n < max_active_block; n++)
         if ((bp = &blocks[n])->number >= 0)
            fprintf(fp, "%d %d %d %d\n", bp->level, bp->cen[0],
                                         bp->cen[1], bp->cen[2]);
      for (i = 1; i < num_pes; i++) {
         RDMA_Send(&size, 1, R_TYPE_INT, i);
         RDMA_Recv(&size, 1, R_TYPE_INT, i);
         if (size > buf_size) {
            if (i != 1)
               free(buf);
            buf_size = size;
         ...
         RDMA_Send(&size, 1, R_TYPE_INT, i);
         RDMA_Recv(buf, 4*size, R_TYPE_INT, i);
         fprintf(fp, "%d\n", size);
      ...
      RDMA_Recv(&size, 1, R_TYPE_INT, 0);
      RDMA_Send(&total_num_blocks, 1, R_TYPE_INT, 0);
      RDMA_Recv(&size, 1, R_TYPE_INT, 0);
      RDMA_Send(buf, 4*total_num_blocks, R_TYPE_INT, 0);
   }
}
